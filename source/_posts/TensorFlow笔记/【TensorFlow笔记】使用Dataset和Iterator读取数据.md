...
# tf.data.Dataset建立数据pipeline

本节分别讲述了在eager和session模式下迭代消耗tf.data.Dataset数据的实例。
<!-- more -->
...
## 总结
**tf.data.Dataset.shuffle函数的reshuffle_each_iteration参数：**
reshuffle_each_iteration 为True时，也只能在buffer_size中打乱数据，而不能在一个epoch完成后打乱整个数据集


# 参考文献
[1] [TensorFlow全新的数据读取方式：Dataset API入门教程](https://zhuanlan.zhihu.com/p/30751039)
[2] [TensorFlow导入数据](https://www.tensorflow.org/guide/datasets?hl=zh-CN)
[3] [tf.data.Dataset.shuffle(buffer_size)中buffer_size的理解](https://juejin.im/post/5b855d016fb9a01a1a27d035)
