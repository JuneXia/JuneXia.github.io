---
title: 【深度学习笔记2.3】VGG（no weight-decay）
date: 2018-02-06 17:28:05
tags:
categories: ["深度学习笔记"]
mathjax: true
---
...
&emsp; vgg16共有3个fc-layer，这里在前两个fc-layer后面都使用dropout，即对vgg16\_1.py中的fc\_layers做如下改进（此部分完整代码详见文献[2]vgg16\_2.py）：
```python
    def fc_layers(self):
        self.fc6 = self.fc("fc1", self.pool5, 4096)
        self.fc6 = tf.nn.dropout(self.fc6, 0.5)
...
在vgg16\_1.py的基础上做如下改动：（此部分代码详见vgg16\_3.py）
```python
# batch_norm定义同文献[3]
